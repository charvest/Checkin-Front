...
const DISCLAIMER_TIMER_SECONDS = 10;
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <motion.button
            ...
            type="button"
          >
            {locked ? `${confirmLabel} (${safeSeconds}s)` : confirmLabel}
          </motion.button>
        </div>
      </div>
    </ModalShell>
...
export default function PHQ9() {
  ...
  const [preDisclaimerSeconds, setPreDisclaimerSeconds] = useState(
    DISCLAIMER_TIMER_SECONDS,
  );
  const [postDisclaimerSeconds, setPostDisclaimerSeconds] = useState(
    DISCLAIMER_TIMER_SECONDS,
  );
  ...
  const disclaimerAccepted = useMemo(() => {
    if (disclaimerAcceptedAt === null || disclaimerAcceptedAt === undefined)
      return false;
    if (!lastSubmittedAt || typeof lastSubmittedAt !== "number") return true;
    return disclaimerAcceptedAt > lastSubmittedAt;
  }, [disclaimerAcceptedAt, lastSubmittedAt]);
  ...
  useEffect(() => {
    ...
    const savedDisclaimerRaw = safeParseJSON(
      STORAGE.getItem(scopedKeys.disclaimerAccepted),
      null,
    );
    const savedDisclaimerAt =
      typeof savedDisclaimerRaw === "number"
        ? savedDisclaimerRaw
        : savedDisclaimerRaw === true
          ? 0
          : null;

    setAnswers(savedAnswers);
    setTermsAccepted(savedTerms);
    setDisclaimerAcceptedAt(savedDisclaimerAt);
    setNowMs(Date.now());
  ...
  useEffect(
    () =>
      STORAGE.setItem(
        scopedKeys.disclaimerAccepted,
        JSON.stringify(disclaimerAcceptedAt),
      ),
    [scopedKeys.disclaimerAccepted, disclaimerAcceptedAt],
  );
  useEffect(
    () =>
  ...
  useEffect(() => {
    if (!isBrowser()) return;
    if (!disclaimerPreOpen) return;
    setPreDisclaimerSeconds(DISCLAIMER_TIMER_SECONDS);
  ...
  useEffect(() => {
    ...
    setPostDisclaimerSeconds(DISCLAIMER_TIMER_SECONDS);
    const id = window.setInterval(
  ...
  return (
    ...
    >
      ...
      <TimedDisclaimerModal
        ...
        onConfirm={() => {
          setDisclaimerAcceptedAt(Date.now());
